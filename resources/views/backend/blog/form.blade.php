    ...
    <div class="box">
        ...
        <div class="box-body">
            <div class="form-group {{ $errors->has('post_type_id') ? 'has-error' : '' }}">
                {!! Form::select('post_type_id', App\Models\PostType::pluck('title', 'id'), null, ['class' => 'form-control', 'placeholder' => 'Choose category']) !!}

                @if($errors->has('post_type_id'))
                    <span class="help-block">{{ $errors->first('post_type_id') }}</span>
                @endif
            </div>
        </div>
    </div>
